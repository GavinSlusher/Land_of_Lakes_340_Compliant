...
CREATE TABLE `clients` (
  ...
  FOREIGN KEY (`address_id`) NOT NULL REFERENCES `addresses`(`address_id`)
  FOREIGN KEY ('client_account_id') NOT NULL REFERENCES 'clients_accounts'('clients_accounts'),
  FOREIGN KEY ('client_advisor_id') NOT NULL REFERENCES 'clients_advisors'("client_advisor_id")
); 
...
CREATE TABLE `accounts` (
  ...
  FOREIGN KEY (`client_id`) NOT NULL REFERENCES `clients`(`client_id`)
); 
...
CREATE TABLE `clients_accounts` (
  ...
  FOREIGN KEY ('account_id') NOT NULL REFERENCES 'accounts'('account_id'),
  FOREIGN KEY ('client_id') NOT NULL REFERENCES 'clients'('client_id')
); 
...
CREATE TABLE `clients_advisors` (
  ...
  FOREIGN KEY (`advisor_id`) NOT NULL REFERENCES `clients`(`advisor_id`),
  FOREIGN KEY (`client_id`) NOT NULL REFERENCES `clients`(`client_id`)
); 
